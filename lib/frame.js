...
class MyAlgoWalletWithIframe extends MyAlgoWallet {
	...
	async verify(password) {
		...
		const res = await this._bridge.sendMessage(
			this._frameHub, { method: "verify", params: { password: password } }, this._url,
			{ waitForReply: true, timeout: 4000 }
		);
	 ...
	 * @async
	 * @access public
	 * @description Get iframe status.
	 * @returns {Status} Returns the status object
	 */
	async status() {
		const res = await this._bridge.sendMessage(
	 ...
	 * @returns {Promise<StoredAccount[]>} Accounts list
	 */
	async getAccounts() {
		if (!this._isSuscribed)
	...
	/**
	* @access public
	* @param {any} tx to be signed. In order to transfere it, it should be encoded.
	* @param {string} password wallet account password
	* @param {string} walletId wallet account id
	* @description send transaction to be signed
	* @returns {SignedTx} tx signed
	*/
	async signTx(tx, password, accountId) {

		if (!this._isSuscribed)
			await this._subscribe();

		const res = await this._bridge.sendMessage(
			this._frameHub, { method: "signTx", params: { tx: tx, password, accountId } }, this._url,
			{ waitForReply: true, timeout: 4000 }
		);
	...
	async getSettings() {
		...
		// Connecting to iframe
		const res = await this._bridge.sendMessage(
			this._frameHub, { method: "settings" }, this._url,
			{ waitForReply: true, timeout: 4000 }
		);
