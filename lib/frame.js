...
class MyAlgoWalletWithIframe extends MyAlgoWallet {
	 ...
	 * @description get all information for given
	 * @returns {void}
	 */
	async getFullAccountInfo(accountId) {
		...
		const res = await this._bridge.sendMessage(
			this._frameHub, { method: "getFullAccountInfo", params: { accountId: accountId } }, this._url,
			{ waitForReply: true, timeout: 4000 }
		);
	...
	async hasAccount() {
		...
		const res = await this._bridge.sendMessage(
			this._frameHub, { method: "hasAccount" }, this._url,
			{ waitForReply: true, timeout: 4000 }
		);
