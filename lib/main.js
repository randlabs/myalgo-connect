...
class MyAlgoConnect {
	...
	/**
	 * @constructor
	 * @param {string} [bridgeUrl] Override wallet.myalgo.com default frame url.
	 */
	constructor(bridgeUrl) {
		 ...
		 */
		this.url = (bridgeUrl ? bridgeUrl : "https://wallet.myalgo.com/bridge");
	 ...
	 */
	async connect(settings = { shouldSelectOneAccount: false }, options = { timeout: 1600000 }) {
		try {
			this.checkPopup(this.currentConnectPopup);
			this.currentConnectPopup = openPopup(this.url + "/connect.html");

			await this._waitForWindowToLoad(this.currentConnectPopup);
			...
			const res = await this.bridge.sendMessage(
				...
				this.url, { waitForReply: true, timeout: options.timeout }
			);

			this.closePopup(this.currentConnectPopup);
	...
	async signTransaction(transaction, options = { timeout: 1600000 }) {
		...
		try {
			this.checkPopup(this.currentSigntxPopup);
			this.currentSigntxPopup = openPopup(this.url + "/signtx.html");
			await this._waitForWindowToLoad(this.currentSigntxPopup);
			...
			this.closePopup(this.currentSigntxPopup);
	...
	async signLogicSig(logic, address, options = { timeout: 1600000 }) {
		...
		try {
			this.checkPopup(this.currentSignLogicSigPopup);
			this.currentSignLogicSigPopup = openPopup(this.url + "/logicsigtx.html");
			await this._waitForWindowToLoad(this.currentSignLogicSigPopup);
			...
			this.closePopup(this.currentSignLogicSigPopup);
	 ...
	 * @param {Window} win Window opened context.
	 * @returns {void}
	 */
	closePopup(win) {
		if (win) win.close();
