 ...
 * @typedef Transaction
 * @property {TxType} type Transaction type
 ...
 * @property {Uint8Array} [note] Transaction note
...
class MyAlgoWallet {
	...
	async signTransaction(transaction, options = { timeout: 1600000 }) {
		...
		if (Array.isArray(transaction)) {
			...
			if (transaction.note.constructor === Uint8Array)
				auxTx.note = Buffer.from(auxTx.note).toString("base64");
		}
	...
	async signLogicSig(logic, address, options = { timeout: 1600000 }) {
		...
		try {
			signLogicSigPopup = openPopup(this._url + "/logic.html");
