...
class MyAlgoConnect {
	 ...
	 */
	async connect(settings = { allowSelectOneAccount: false }, options = { timeout: 1600000 }) {
		try {
			...
			const res = await this._bridge.sendMessage(
				...
				this._url, { waitForReply: true, timeout: options.timeout, allowSelectOneAccount: settings.allowSelectOneAccount }
			);
