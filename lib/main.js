 ...
 * @property {boolean} [disableLedgerNano] Disable Ledger Nano accounts, it will return only mnemonic accounts
 */

/**
 * @description Sign txn settings
 * @typedef SignTxSettings
 * @type {object}
 * @property {boolean} [disableLedgerNano] Disable Ledger Nano accounts, it will not allow users to sign with ledger accounts
 */
...
class MyAlgoConnect {
	 ...
	 */
	async connect(settings = { shouldSelectOneAccount: false, disableLedgerNano: false }) {
	 ...
	 * @param {SignTxSettings} settings Sign tx settings
	 ...
	 */
	async signTransaction(transaction, settings = { disableLedgerNano: false }) {
		let txn;
		...
		try {
			...
			const res = await this.bridge.sendMessage(
				...
				{ method: "transaction", params: { txn, settings } },
				this.url, this.options
			);
