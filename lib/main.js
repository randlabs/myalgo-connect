...
function prepareTxn(txn) {
	...
	if (txn.group && (txn.group.constructor === Buffer || txn.group.constructor === Uint8Array))
		txn.group = txn.group.toString("base64");
	return txn;
}
...
class MyAlgoWallet {
	...
	async signTransaction(transaction, options = { timeout: 1600000 }) {
		...
		if (Array.isArray(transaction))
			txn = Array.from(transaction).forEach(tx => prepareTxn(tx));
		else
